...
FROM
`news` INNER JOIN `category` ON `news`.categori_id=`category`.`id` WHERE `category`.`title`='Google' Limit $start,$limit";
...
<nav style="margin-left: 550px;">
    <ul class="pagination">
        <?php
        $sql="Select * from news WHERE categori_id=1";
        $result=mysqli_query($dbConnection,$sql);
        ...
        for ($i = 1; $i <= $totalPageCount; $i++) {
            ...
            echo '<li><a href="http://localhost/ACA/Exam/Exam/Google.php?page=' . $i . '" style="' . $style . '">' . $i . '</a></li>';
        }
        ?>
